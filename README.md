...
A Windows Geode mod MVP that lets you select a `.gmd`, `.gmd2`, or `.lvl` file, parses it through `hjfod.gmd-api`, searches a small number of Geometry Dash **Recent** pages, downloads the candidate levels sequentially, and ranks them by structural similarity.
...
The comparison currently uses object ID, position, rotation, scale, flips, translation normalization, 300-unit sections, multiset Jaccard similarity, and partial-section matching. Color/group/trigger metadata is ignored by the parser for now.
...
5. The mod queries Recent pages, downloads candidates one at a time, then shows the top matches.
...
- `kMaxCandidates` — hard cap on downloaded candidates;
- `kMaxDisplayedResults` — number of results shown.
